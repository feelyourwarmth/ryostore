...
// Wallhaven search/download logic. Loaded once as the plugin's `main` entry point
// and kept alive while the popout opens and closes, so results and paging survive.
// The framePanel (ui/Panel.qml) reads this through pluginApi.mainInstance.
Item {
  id: root
  ...
  readonly property var commandEnv: apiKey.length > 0 ? ({
                                                           "WALLHAVEN_API_KEY": apiKey
                                                         }) : ({})
