#!/usr/bin/env python3
"""Import curated public-domain decors into the RyoStore catalogue.

A decor is a curated public-domain image seeded into ~/Pictures/ryodecors, where
the Ryoku Hub's Decor/Placard components display it. Every decor ships two PNG
variants: the raw image (`content/source.png`) and a pre-baked dithered variant
(`content/dither.png`, Ryoku's one-bit bone-on-transparent look). The store's
install offers a dither toggle to pick which variant lands on the machine.
...
Two phases keep it reproducible:
  ...
  fetch  - resolve each source through the Wikimedia Commons API or the Met
           ...
           the long edge as decors/<id>/content/source.png, and write a
           PROVENANCE.txt (source URL + licence + attribution) at the product
           root.

  build  - bake decors/<id>/content/dither.png with bin/art/ryodither (a Bayer
           dither to Ryoku bone on transparent; --invert inks dark-on-light
           sources), render assets/preview.webp by compositing that dither
           centred on a >=1280x720 #101010 canvas with `magick`, sample an accent
           tone from the source, write each manifest.json (real byte sizes +
           sha256; source/dither/PROVENANCE install:true, preview install:false;
           destination ryodecors/<id>) and rewrite decors/registry.json with
           matching manifestSha256 values.

Usage:
    tools/import-decors.py fetch          # download + normalise every source
    tools/import-decors.py build          # bake dithers + previews + registry
    tools/import-decors.py all            # fetch then build
    tools/import-decors.py ids            # print curated ids
...
BONE = "#e8d8c9"                      # Ryoku bone: the decor ink colour
SURFACE = "#101010"                   # the dark ground the decor sits on
...
USER_AGENT = "ryoku-import-decors/1.0 (RyoStore catalogue tooling)"

# Curated set: public-domain / CC0 art that dithers into a striking one-bit bone
# decor and stays distinct from the shipped set. `invert` inks the dark tones,
# for dark-on-light sources (engravings, prints, the bronze on a light ground).
CURATED = [
    dict(
        id="melencolia", name="Melencolia I", source="commons", invert=True,
        title="File:Albrecht Dürer - Melencolia I - Google Art Project.jpg",
        author="Albrecht Dürer (1514)",
        summary="Dürer's brooding angel of genius, drawn in bone stipple.",
        description="Albrecht Dürer's 1514 master engraving Melencolia I - the winged figure, the magic square, and the scattered instruments of thought - reduced to Ryoku's one-bit bone line.",
        tags=["engraving", "durer", "classical", "monochrome"],
    ),
    dict(
        id="rhinoceros", name="The Rhinoceros", source="commons", invert=True,
        title="File:Dürer's Rhinoceros, 1515.jpg",
        author="Albrecht Dürer (1515)",
        summary="Dürer's armoured rhinoceros woodcut, one-bit and bone.",
        description="The Rhinoceros (1515), Dürer's famous woodcut of a beast he never saw, plated in riveted armour - baked to bone-on-transparent stipple.",
        tags=["woodcut", "durer", "animal", "monochrome"],
    ),
    dict(
        id="carceri", name="Carceri", source="commons", invert=True,
        title="File:Giovanni Battista Piranesi - Le Carceri d'Invenzione - First Edition - 1750 - 07 - The Drawbridge.jpg",
        author="Giovanni Battista Piranesi (c.1750)",
        summary="A Piranesi imaginary prison vaulting into bone shadow.",
        description="The Drawbridge, a plate from Piranesi's Carceri d'Invenzione (c.1750) - impossible stairs, bridges, and engines of stone - dithered into stark Ryoku bone.",
        tags=["etching", "piranesi", "architecture", "dramatic"],
    ),
    dict(
        id="red-fuji", name="Red Fuji", source="commons", invert=True,
        title="File:Red Fuji southern wind clear morning.jpg",
        author="Katsushika Hokusai (c.1830)",
        summary="Hokusai's Red Fuji under a streaked morning sky.",
        description="Fine Wind, Clear Morning (Red Fuji) from Hokusai's Thirty-six Views of Mount Fuji - the bare peak and its cloud-banded sky - reduced to Ryoku's one-bit bone.",
        tags=["woodblock", "hokusai", "landscape", "ukiyo-e"],
    ),
    dict(
        id="pillars", name="Pillars of Creation", source="commons", invert=False,
        title="File:Pillars of Creation.jpg",
        author="NASA, ESA / STScI (Hubble)",
        summary="The Eagle Nebula's Pillars of Creation in bone light.",
        description="The Hubble Space Telescope's Pillars of Creation - towers of interstellar gas and dust in the Eagle Nebula - dithered so the starlight reads as Ryoku bone.",
        tags=["astronomy", "nasa", "nebula", "space"],
    ),
    dict(
        id="trebonianus", name="Trebonianus Gallus", source="met", invert=True,
        object_id=247117,
        author="Roman bronze, The Met (Open Access, CC0)",
        summary="A bronze Roman emperor, arm raised, cast in bone.",
        description="The Metropolitan Museum's over-life-size bronze of the emperor Trebonianus Gallus (251-253 CE), his silhouette reduced to Ryoku's one-bit bone.",
        tags=["sculpture", "bronze", "roman", "classical"],
    ),
    dict(
        id="muybridge-deer", name="Deer in Motion", source="commons", invert=False,
        title="File:Eadweard Muybridge - Animal Locomotion, Plate 686 - Google Art Project.jpg",
        author="Eadweard Muybridge (1887)",
        summary="Muybridge's deer, caught mid-stride across the frame.",
        description="A plate from Eadweard Muybridge's Animal Locomotion (1887) - fallow deer stepping through a wooded run, frame by frame - baked to Ryoku bone.",
        tags=["muybridge", "motion", "animal", "sequence"],
    ),
]
...
def selected(ids: list[str]) -> list[dict]:
    ...
    for wanted in ids:
        if wanted not in CURATED_BY_ID:
            raise SystemExit(f"unknown decor id: {wanted}")
...
def provenance_text(entry: dict, meta: dict) -> str:
    return (
        ...
        "assets/preview.webp is catalogue media (the dithered look on a dark "
        "ground) and is not installed with the runtime payload.\n"
    )
...
def build(ids: list[str]) -> None:
    ...
    for entry in selected(ids):
        ...
        manifest = {
            ...
            "destination": f"ryodecors/{pid}", "files": files,
        }
    ...
    print(f"registry.json updated: {len(ordered)} decors")


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Import curated public-domain decors into RyoStore")
    ...
    for name, help_text in (("fetch", "download + normalise every source"),
        ...
        cmd.add_argument("ids", nargs="*", help="decor ids (default: all curated)")
